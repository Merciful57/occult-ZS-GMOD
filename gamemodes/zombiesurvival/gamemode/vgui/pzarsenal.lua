local function ScrapLabelThink(self)
	local points = MySelf:Frags()
	if self.m_LastScrap ~= points then
		self.m_LastScrap = points
...
function PANEL:Paint(w, h)
	...
	if oldquality ~= hovquality or oldbranch ~= hovbranch then
		...
		if hovquality and hovbranch then
			...
			self.ScrapCost:SetText(scost ~= "" and "Silver Cost: " .. scost or "")
			self.ScrapCost:SetTextColor(scost ~= "" and MySelf:GetFrags() >= scost and COLOR_WHITE or COLOR_RED)
...
function GM:OpenZArsenalMenu(remantler)
	...
	local title = EasyLabel(topspace, "Zombie Arsenal", "ZSHUDFontSmall", COLOR_WHITE)
	title:CenterHorizontal()
	local subtitle = EasyLabel(topspace, "Purchase Death!", "ZSHUDFontTiny", COLOR_WHITE)
	...
	local pointslabel = EasyLabel(bottomspace, "Silver in your pockets: 0", "ZSHUDFontTiny", COLOR_GREEN)
	pointslabel:AlignTop(4)
	pointslabel:AlignLeft(8)
	pointslabel.Think = ScrapLabelThink

	local lab = EasyLabel(bottomspace, "Upgrades apply to the whole Zombie team!", "ZSHUDFontTiny")
	lab:AlignTop(4)
	lab:AlignRight(4)
	...
	local sheet = remprop:AddSheet("Coming soon", remantleframe, "icon16/arrow_up.png", false, false)
	...
	sheet = remprop:AddSheet("Upgrades", trinketsframe, GAMEMODE.ItemCategoryIcons[ITEMCAT_TRINKETS], false, false)
	sheet.Panel:SetPos(0, tabhei + 2)
	...
	sheet = remprop:AddSheet("Abilities", ammoframe, GAMEMODE.ItemCategoryIcons[ITEMCAT_AMMO], false, false)
	sheet.Panel:SetPos(0, tabhei + 2)
	...
	for frameindex = 0, 1 do
		...
		if frameindex == 0 then
			...
			local subcats = GAMEMODE.ItemZSubCategories
			local tbn
			for j = 1, #subcats do
	...
	self:CreateZItemInfoViewer(trinketsframe, frame, topspace, bottomspace, MENU_REMANTLER)
	...
	local disabtn = EasyButton(remantleframe, "Boss Menu", 8, 4)
...
function GM:CreateZItemInfoViewer(frame, propertysheet, topspace, bottomspace, menutype)
	local __, topy = topspace:GetPos()
	local ___, boty = bottomspace:GetPos()
	local screenscale = BetterScreenScale()
