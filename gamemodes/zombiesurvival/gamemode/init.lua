...
AddCSLuaFile("vgui/pzarsenal.lua")
		...
		for _, pl in pairs(allplayers) do
			if P_Team(pl) == TEAM_HUMAN and P_Alive(pl) then
				...
				if self:GetWave() > 0 and time > (pl.NextResupplyUse or 0) then
					...
					pl.NextResupplyUse = time + self.ResupplyBoxCooldown * (pl.ResupplyDelayMul or 1) * (stockpiling and 1.95 or 1)
...
function GM:HumanKilledZombie(pl, attacker, inflictor, dmginfo, headshot, suicide)
	...
	attacker:PS2_AddStandardPoints(100, "Killed a zombie")
